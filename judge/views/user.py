...
from judge.forms import DownloadDataForm
from judge.forms import EmailChangeForm
from judge.forms import newsletter_id
...
from judge.utils.celery import task_status_by_id
...
class UserPage(TitleMixin, UserMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['authored'] = (
            self.object.authored_problems.filter(is_public=True, is_organization_private=False)
...
class EmailChangeRequestView(LoginRequiredMixin, TitleMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if cache.incr(key) > settings.DMOJ_EMAIL_CHANGE_LIMIT_COUNT:
            return HttpResponse(
                _('You have sent too many email change requests. Please try again later.'),
                content_type='text/plain',
                status=429,
            )
        return super().post(request, *args, **kwargs)
